...
static Animation spider_animations[] =
{
  Animation(attack_frames, 2),
  Animation(walk_frames, 5),
  Animation(rise_frames, 3),
  Animation(awaken_frames, 12),
  Animation(sleep_frames, 1),
};

void Spider::setState(uint8_t state)
{
    _animationPlayer.setLooped(false);
    _state = state;
    
    switch (state)
    {
        case SPIDER_SLEEP:
            _animationPlayer.setLooped(true);
            setAnimation(SPIDER_ANIMATION_SLEEP);
            break;
        case SPIDER_AWAKEN:
            setAnimation(SPIDER_ANIMATION_AWAKEN);
            break;
        case SPIDER_RISE:
            setAnimation(SPIDER_ANIMATION_RISE);
            break;
        case SPIDER_WALK:
            _animationPlayer.setLooped(true);
            _movement = -1;
            setAnimation(SPIDER_ANIMATION_WALK);
            break;
    }
}

void Spider::setAnimation(int index)
{
    assert(index >= 0 && index < SPIDER_ANIMATIONS_COUNT);
    _animationPlayer.setAnimation(&spider_animations[index]);
}